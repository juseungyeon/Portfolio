const PortUi = (function() {
    ...
    const _addEvent = function(){
        window.addEventListener('scroll',_setScrollMotion.workScroll);
    }

    const workEl = document.querySelectorAll('.work > div');

    const _setScrollMotion = {
        scrollVal : window.pageYOffset,
        windowH : window.innerHeight,
        workEl : document.querySelectorAll('.work > div'),
        contact : document.querySelector('.contact'),
        activeBg : document.querySelector('.trans-bg'),

        workScroll : function (){
            console.log(workEl);
            workEl.forEach(works => {
                if(scrollVal >=  works.offsetTop + windowH / 1.2) {
        ...
        // contact : function(){
        //     if(this.scrollVal >= this.contact.offsetTop - this.windowH / 1.8){
        //         this.activeBg.classList.add('up');
        //     }
        //     else {
        //         this.activeBg.classList.remove('up');
        //     }
        // }
    ...
    mainScroll: function (obj) { 0
        ...
        function event01() {
            $workEl.each(function(){
                if(scrollVal >= $(this).offset().top - $(window).height() / 1.5){
                    $(this).addClass('view');
                }
            });
        }
        function event02() {
            ...
            if(scrollVal>= $work.offset().top) {
                $('.h-text').css({ 'transform': 'translate3d(0px, -2%, 0px) skew(0deg, 6deg)', 'opacity': 0});
            }
        }
        function event03() {
            if(scrollVal >= $contact.offset().top - $(window).height() / 1.8){
    ...
    subScroll: function (obj) { 0
        ...
        function event01() {
            $showCon.each(function(){
                if(scrollVal >= $(this).offset().top - $(window).height() / 1.2){
                    $(this).addClass('active');
                }
            }); 
        }
        function event02() {
            if(scrollVal >= $paging.offset().top - $(window).height() / 1){
                $('.trans-bg').addClass('up');
            }
            else {
                $('.trans-bg').removeClass('up');
